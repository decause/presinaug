...
@app.route("/table")
def table():
    #print type(periodictable)
    #for item in periodictable:
    #    print item
    print wordfreq
    return render_template('css3d_periodictable.mak', name='mako', periodictable=periodictable, beeriodictable=beeriodictable, wordfreq=wordfreq)


@app.route("/blog")
def posts():
    return render_template('bootflat.mak', name='mako')


@app.route('/resources')
def resources():
    resources = dict()
    resources['posts'] = os.listdir(os.path.join(base_dir, 'static', 'posts'))
